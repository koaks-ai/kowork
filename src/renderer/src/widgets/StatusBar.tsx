...
export function StatusBar({ bootstrap, frosted = false }: { bootstrap: AppBootstrapDto; frosted?: boolean }): React.JSX.Element {
  const { t } = useTranslation()
  const { projectId, threadId } = useWorkbenchStore()
  const project = bootstrap.projects.find((item) => item.id === projectId)
  return (
    <footer data-frosted={frosted || undefined} className="kw-chrome flex h-6 shrink-0 items-center justify-between border-t border-kw-border-default px-3 text-[10px] text-kw-text-muted">
