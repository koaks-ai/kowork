...
describe('MarkdownContent', () => {
  ...
  it('keeps highlighted code but mutes colors for reasoning tone', () => {
    ...
    expect(root?.className).toContain('text-kw-text-muted')
    expect(view.container.querySelector('h3')?.className).toContain('text-kw-text-muted')
    expect(view.container.querySelector('strong')?.className).toContain('text-kw-text-muted')
    expect(view.container.querySelector('a')?.className).toContain('text-kw-text-muted')
    expect(code?.classList.contains('hljs')).toBe(true)
    expect(code?.querySelector('.hljs-keyword')?.textContent).toBe('const')
  })
